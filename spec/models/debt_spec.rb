...
describe Debt do
  ...
  describe 'Validations' do
    describe 'responds to ' do
      characteristics = %i[permit_infraction_number amount_owed_pending_balance paid_in_full
    ...
    it 'does not allow amount owed pending balance to be nil' do
      debt1 = FactoryGirl.build(:debt, amount_owed_pending_balance: nil)
      expect(debt1).not_to be_valid
    end

    it 'is invalid without a debtor id' do
      expect(described_class.new(debtor_id: nil)).not_to be_valid
    end
    ...
    it 'has a 9 digit routing number or is blank' do
      debt3 = FactoryGirl.build(:debt, bank_routing_number: rand(1_111_111_111..9_999_999_999))
      debt3.valid?
    ...
    it 'orginating date set to nil should not be valid' do
      expect(FactoryGirl.build(:debt, original_debt_date: nil)).not_to be_valid
    end
  end
  ...
  describe 'Method Specs' do
    describe 'find_debtor_attribute works' do
      ...
      it 'returnses array of name and contact name' do
        FactoryGirl.create(:debtor, contact_person: 'Samuel Clemens', name: 'Mark Twain', id: 236)
        expect(subject.find_debtor_attr(236, %i[name contact])).to eq(['Mark Twain', 'Samuel Clemens'])
      end
      it 'fails if given anything other to search for' do
    ...
    describe 'CSV creation' do
      it "does not error'" do
        expect { described_class.to_csv }.not_to raise_error
      end
      it 'creates a valid csv file' do
        expect(described_class.to_csv.chomp).to eq(
