...
class Debtor < ActiveRecord::Base
  ...
  before_save { self.contact_person_email = email.downcase if email }
  before_save do
    self.ss_hex_digest = unless ss_hex_digest.blank?
                           Debtor.encrypt(ss_hex_digest)
                         end
  end
  # TODO: before save: clean up telephone number and ss
  before_save do
    self.tel = Debtor.remove_hyphens(tel) if tel
  end
